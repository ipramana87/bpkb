...
class Recheckbpkb extends CI_Controller {
	...
	public function index() {
		$this->load->view('vrecheckbpkb');
	}

	public function gridcheck() {
		$sCari = trim($this->input->post('fs_cari'));
		$nStart = trim($this->input->post('start'));
		$nLimit = trim($this->input->post('limit'));

		$this->db->trans_start();
		$this->load->model('MReChekBPKB');
		$sSQL = $this->MReChekBPKB->listCheckAll($sCari);
		$xTotal = $sSQL->num_rows();
		$sSQL = $this->MReChekBPKB->listCheck($sCari, $nStart, $nLimit);
