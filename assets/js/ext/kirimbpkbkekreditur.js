...
Ext.onReady(function() {
	...
	var winGrid = Ext.create('Ext.grid.Panel', {
		...
		listeners: {
			itemdblclick: function(grid, record) {
				...
				Ext.getCmp('txtKdTransaksi').setValue(record.get('fs_transaksi'));
				winCari.hide();
			}
		},
		viewConfig: {
	...
	var grupKreditur = Ext.create('Ext.data.Store', {
		autoLoad: true,
		...
		proxy: {
			...
			type: 'ajax',
			url: 'terimadarikreditur/gridkreditur'
		},
		listeners: {
			beforeload: function(store) {
				Ext.apply(store.getProxy().extraParams, {
	...
	var winGrid1 = Ext.create('Ext.grid.Panel', {
		...
		columns: [
			{xtype: 'rownumberer', width: 45},
			{text: "Kode Kreditur", dataIndex: 'fs_kode_kreditur', menuDisabled: true, flex: 1},
			{text: "Nama Kreditur", dataIndex: 'fs_nama_kreditur', menuDisabled: true, flex: 1},
		],
		tbar: [{
			flex: 1.4,
			layout: 'anchor',
		...
		listeners: {
			itemdblclick: function(grid, record) {
				Ext.getCmp('cboKreditur').setValue(record.get('fs_nama_kreditur'));
				winCari1.hide();
			}
		},
		viewConfig: {
	...
	var frmKirimBPKBDariKreditur = Ext.create('Ext.form.Panel', {
		border: false,
		frame: true,
		region: 'center',
		title: 'Kirim BPKB Dari Kreditur',
		width: 930,
		items: [{
			activeTab: 0,
	...
	var vMask = new Ext.LoadMask({
		msg: 'Please wait...',
		target: frmKirimBPKBDariKreditur
	});

	function fnMaskShow() {
		frmKirimBPKBDariKreditur.mask('Please wait...');
	}

	function fnMaskHide() {
		frmKirimBPKBDariKreditur.unmask();
	}
	
	frmKirimBPKBDariKreditur.render(Ext.getBody());
